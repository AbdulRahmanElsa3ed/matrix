...
async function startServer() {
  ...
  const DATA_FILE = import_path.default.join(DATA_DIR, "app-data.json");
  ...
  app.post("/api/data", (req, res) => {
    try {
      ...
      import_fs.default.writeFileSync(DATA_FILE, JSON.stringify(dataToSave, null, 2), "utf-8");
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`\u{1F680} Production/Dev Server running on http://0.0.0.0:${PORT}`);
  });
}
startServer();
